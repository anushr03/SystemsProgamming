/*------------------------------------
  ...
  Did not attempt the extra credit part
*------------------------------------- */
...
void* mm_malloc (size_t size) {
  ...
  ptrFreeBlock = (BlockInfo*)searchFreeList(reqSize);// find a free block that is big enough

  if (ptrFreeBlock == NULL) { // Search freeList & check return value
  ...
  if (blockSize - reqSize >= MIN_BLOCK_SIZE) { // Check if block is big enough to split

    ptrFreeBlock->sizeAndTags = reqSize | TAG_USED | precedingBlockUseTag; // Set size, tag, and use tag
    ptrNextBlock = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize); // Get next block
    ptrNextBlock->sizeAndTags = blockSize - reqSize | TAG_PRECEDING_USED; // Set size and use tag
    *((size_t*)UNSCALED_POINTER_ADD(ptrNextBlock, blockSize - reqSize - WORD_SIZE)) = blockSize - reqSize | TAG_PRECEDING_USED; // Set boundary tag

    insertFreeBlock(ptrNextBlock); // Add second block to freeList
    coalesceFreeBlock(ptrNextBlock); // Coalesce second block with adjacent free blocks

  }
  else { // Block is not big enough to split
    // Update block footer
    size_t *ptrFreeBlockFooter = (size_t *) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize - WORD_SIZE); // Get footer
    *ptrFreeBlockFooter = *ptrFreeBlockFooter | TAG_USED; // Set footer to used
  ...
  return UNSCALED_POINTER_ADD(ptrFreeBlock, WORD_SIZE); // Return pointer to block
...
void mm_free(void *ptr) {
  ...
  followingBlock = (BlockInfo*) UNSCALED_POINTER_ADD(blockInfo, SIZE(blockInfo->sizeAndTags)); // Get following block info

  payloadSize = SIZE(blockInfo->sizeAndTags) - WORD_SIZE; // Get payload size

  if ((followingBlock->sizeAndTags & TAG_USED) == 0) { // Check if following block is free
    *((size_t*) UNSCALED_POINTER_ADD(followingBlock, SIZE(followingBlock->sizeAndTags) - WORD_SIZE)) = followingBlock->sizeAndTags; // Update following block footer
  }

  blockInfo->sizeAndTags = blockInfo->sizeAndTags & ~TAG_USED; // Set block to free
  *((size_t*) UNSCALED_POINTER_ADD(blockInfo, payloadSize)) = blockInfo->sizeAndTags; // Update block footer
  followingBlock->sizeAndTags &= ~TAG_PRECEDING_USED; // Update following block's header/footer preceding tag bit
